...
class NavigationTrajectorySampler(Sampler[int]):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        for traj in traj_dict:
            traj_dict[traj].sort(key=lambda x: x[0])

        traj_keys = list(traj_dict.keys())
        random.shuffle(traj_keys)

        for traj in traj_keys:
            yield from (idx for _, idx in traj_dict[traj])


class Qwen2VLNavGRPOTrainer(Trainer):
    """
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if (
                ...
                raise ValueError(
                    "Invalid `torch_dtype` passed to `GRPOConfig`. Expected either"
                    " 'auto' or a string representing a `torch.dtype` (e.g.,"
                    f" 'float32'), but got {torch_dtype}."
                )
            ...
            if args.model_init_kwargs is not None:
                raise ValueError(
                    "You passed `model_init_kwargs` to the `GRPOConfig`, but your model"
                    " is already instantiated. This argument can only be used when the"
        ...
        if reward_processing_classes is None:
            ...
            if len(reward_processing_classes) != len(reward_funcs):
                raise ValueError(
                    "The number of reward processing classes must match the number of"
                    " reward functions."
                )
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        ...
        return NavigationTrajectorySampler(self.train_dataset)
